...
function takeOrder(customerName: string, order: string) {
    ...
    if (specificCustomer == undefined) {
        ...
        if (!food.find(x => x.name.toLowerCase().includes(order.toLowerCase()))) {
            ...
            if (orderedFoodIngredients != undefined) {
                ...
                if (alergies != undefined && alergies.some(x => matching.includes(x))) {
                    return `${specificCustomer.customerName} - ${orderedFood?.name}: can't order, alergic to: ${alergies}`;
                } else {
