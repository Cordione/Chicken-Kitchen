...
export function possibleOutputs(command: string, alergies: string[], specificCustomer: ICustomerAlergies, orderedFood: IFood, orderCost: number, matching: string[], restaurant: IRestaurant) {
    ...
    if (command.toLowerCase() == 'Buy'.toLowerCase()) {
        if (matchingAlergies && specificCustomer.budget < orderCost) {
            ...
            const output = `${specificCustomer.customerName} have budget: ${specificCustomer.budget} -> wants to order ${orderedFood?.name}, which cost: ${orderCost.toFixed(2)}: success`;
